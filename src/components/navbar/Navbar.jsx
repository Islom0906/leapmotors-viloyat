...
const Navbar = () => {
  ...
  const { data } = useQuery("get-model", () => apiService.getData("/product"));
    ...
    <nav
      className={`${
        sale.asPath == "/car-sale"
          ? "bg-white shadow-sm"
          : "bg-[rgba(27,27,27,0.85)]"
      } w-full fixed  top-0 left-0 h-[60px] z-[999]  `}
    >
      <div
        className={` ${
          sale.asPath == "/car-sale" ? "md:container px-1" : "container"
      ...
      >
        <div className="flex items-center justify-between sm:gap-20">
          <Link
            href="/"
            className={` ${
              sale.asPath == "/car-sale" ? "w-[35px]" : "w-[154px]"
            }  h-[34px] relative`}
          >
            {sale.asPath == "/car-sale" ? (
              <Image
                src={
                  "https://leapmotorca.uz/api/media/5c50e291-35be-46e2-8d99-f5780c1377b9-Leapmotor-logo.png"
                }
                alt="Leapmotor"
          ...
          <ul
            className={`${
              sale.asPath == "/car-sale" ? "hidden" : "lg:flex"
            } items-center hidden gap-10 text-white uppercase `}
          >
            {sortProduct?.map((link) => (
              <li key={link?._id}>
                <Link
                  href={`/${link?.model}`}
                  className="hover:underline hover:text-[#4f5f82] transition-all ease duration-500 underline-offset-8 font-semibold"
                >
                  {link?.model}
                </Link>
              </li>
        ...
        <div className="flex items-center md:gap-14">
          <a
            ...
            className=" border-white border px-4 py-1 text-white hover:text-[#4f5f82] hover:bg-white  transition-all ease duration-500 underline-offset-8 font-arial-semibold hidden lg:block"
          >
            {t("navbar.testDrive")}
          </a>
          <div className="side-r">
            <GiHamburgerMenu
              onClick={(e) => showSidebar(e, !show)}
              className="text-2xl text-white cursor-pointer"
            />
            <div
              className={`backdrop-brightness-50 w-full h-screen fixed top-0 left-0 right-0 bottom-0 ${
                show ? "block" : "hidden"
            ...
            <div
              ...
              }  duration-500 ease-in-out bg-white w-full xs:w-[200px] h-screen pt-5 pb-10 px-5 text-[#808080] shadow-2xl flex flex-col justify-between z-[99]`}
            >
              <div>
                ...
                <ul className="flex flex-col items-center space-y-5 md:items-start">
                  {data?.data?.map((link) => (
                    <li key={link?._id} onClick={(e) => showSidebar(e, false)}>
                      <Link
                        href={`/${link?.model}`}
                        className="block uppercase lg:hidden "
                      >
                        {link?.model}
                      </Link>
                    </li>
                  ...
                  <li>
                    <Link
                      ...
                      className="capitalize"
                    >
                      {t("navbar.dealersCenter")}
                    </Link>
                  </li>
                  <li>
                    <a
                      onClick={(e) => showSidebar(e, false)}
                      href="https://leapmotorca.uz/dealers"
                      className="capitalize"
                    >
                      {t("navbar.dealers")}
                    </a>
